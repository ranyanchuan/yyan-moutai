...
var yyan = (function(document, undefined) {
	...
	/**
	 * yyan isWindow(需考虑obj为undefined的情况)
	 */
...
(function($, document) {
	function detect(ua) {
		...
		if (plus) {
			this.os.plus = true;
			$(function() {
				document.body.classList.add('yyan-plus');
			});
			if (ua.match(/StreamApp/i)) { //TODO 最好有流应用自己的标识
				this.os.stream = true;
				$(function() {
					document.body.classList.add('yyan-plus-stream');
				});
			}
		}
	}
...
(function($) {
	...
	var delegateFn = function(element, event, selector, callback) {
		return function(e) {
			...
			for (; target && target !== document; target = target.parentNode) {
				...
				if (~['click', 'tap', 'doubletap', 'longtap', 'hold'].indexOf(event) && (target.disabled || target.classList.contains('yyan-disabled'))) {
					break;
				}
				var matches = {};
	...
	$.fn.off = function(event, selector, callback) {
		return this.each(function() {
			var _mid = mid(this);
			if (!event) { //yyan(selector).off();
				delegates[_mid] && delete delegates[_mid];
			} else if (!selector) { //yyan(selector).off(event);
				delegates[_mid] && delete delegates[_mid][event];
			} else if (!callback) { //yyan(selector).off(event,selector);
				delegates[_mid] && delegates[_mid][event] && delete delegates[_mid][event][selector];
			} else { //yyan(selector).off(event,selector,callback);
...
/**
 * yyan target(action>popover>modal>tab>toggle)
 */
...
/**
 * yyan fixed requestAnimationFrame
 * @param {type} window
 * @returns {undefined}
 */
(function(window) {
...
(function($, document) {
	$(function() {
		...
		var CLASS_FOCUSIN = 'yyan-focusin';
		var CLASS_BAR_TAB = 'yyan-bar-tab';
		var CLASS_BAR_FOOTER = 'yyan-bar-footer';
		var CLASS_BAR_FOOTER_SECONDARY = 'yyan-bar-footer-secondary';
		var CLASS_BAR_FOOTER_SECONDARY_TAB = 'yyan-bar-footer-secondary-tab';
...
/**
 * yyan gesture flick[left|right|up|down]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
...
/**
 * yyan gesture swipe[left|right|up|down]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
...
/**
 * yyan gesture drag[start|left|right|up|down|end]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
...
/**
 * yyan gesture tap and doubleTap
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
	...
	var handle = function(event, touch) {
		...
		switch (event.type) {
			case $.EVENT_END:
				...
				if (!target || (target.disabled || (target.classList && target.classList.contains('yyan-disabled')))) {
					return;
				}
...
(function($) {
	...
	var createIframe = function(options) {
		...
		wrapper.className = 'yyan-iframe-wrapper';
	...
	function handleScroll(wrapper, iframe) {
		var key = 'yyan_SCROLL_POSITION_' + document.location.href + '_' + iframe.src;
	...
	$(function() {
		...
		if ($.os.ios) {
			...
			classList.add('yyan-ios');
		} else if ($.os.android) {
			os.push({
				os: 'android',
				version: $.os.version
			});
			classList.add('yyan-android');
		}
		if ($.os.wechat) {
			os.push({
				os: 'wechat',
...
(function($) {
	...
	$.fire = function(webview, eventType, data) {
		if(webview) {
			if(typeof data === 'undefined') {
				...
				webview.evalJS("typeof yyan!=='undefined'&&yyan.receive('" + eventType + "'," + data + ")");
				return;
	...
	$.openWindow = function(url, id, options) {
		...
		if(webview) { //已缓存
			//每次show都需要传递动画参数；
			//预加载的动画参数优先级：openWindow配置>preloadPages配置>yyan默认配置；
				...
				view.addEventListener("click", function(e) {
					...
					if(backClick&& x > backClick.left && x < backClick.right){
						if( _back.click && $.isFunction(_back.click)){
							...
							webview.evalJS("window.yyan&&yyan.back();");
						}
					}
				}, false);
				webview.append(view);
	...
	$.addInit({
		...
		handle: function() {
			...
			if($.os.plus) {
				...
				//					err.className = 'yyan-error';
				//					//文字描述
				//					var span = document.createElement('span');
				//					span.innerHTML = '在该浏览器下，不支持创建子页面，具体参考';
				//					err.appendChild(span);
				//					var a = document.createElement('a');
				//					a.innerHTML = '"yyan框架适用场景"';
	...
	$.ready(function() {
		...
		if($.supportStatusbarOffset()) {
			document.body.classList.add('yyan-statusbar');
		}
	});
})(yyan);
...
(function($, window) {
	...
	window.addEventListener('tap', function(e) {
		...
		if (action && action.classList.contains('yyan-action-back')) {
			$.back();
...
(function($, window) {
	if ($.os.plus && $.os.android) {
		$.addBack({
			...
			handle: function() {
				...
				//popover
				if ($.targets._popover && $.targets._popover.classList.contains('yyan-active')) {
					$($.targets._popover).popover('hide');
					return true;
				}
				//offcanvas
				var offCanvas = document.querySelector('.yyan-off-canvas-wrap.yyan-active');
				if (offCanvas) {
	...
	$.menu = function() {
		var menu = document.querySelector('.yyan-action-menu');
		if (menu) {
									...
									if(window.__NWin_Enable__ === false) { //不支持多webview
										...
										if(hasPulldown) {
											...
											if(parent) {
												...
												parent.evalJS("yyan.fn.pullRefresh=yyan.fn.pullRefresh_native");
												//父页面初始化pulldown
												parent.evalJS("yyan&&yyan(document.querySelector('.yyan-content')).pullRefresh('" + JSON.stringify(downOptions) + "')");
											}
										}
									}
								});
...
/**
 * yyan layout(offset[,position,width,height...])
 * @param {type} $
 * @param {type} window
 * @param {type} undefined
 * @returns {undefined}
...
(function($, document, undefined) {
    var CLASS_PULL_TOP_POCKET = 'yyan-pull-top-pocket';
    var CLASS_PULL_BOTTOM_POCKET = 'yyan-pull-bottom-pocket';
    var CLASS_PULL = 'yyan-pull';
    var CLASS_PULL_LOADING = 'yyan-pull-loading';
    var CLASS_PULL_CAPTION = 'yyan-pull-caption';
    var CLASS_PULL_CAPTION_DOWN = 'yyan-pull-caption-down';
    var CLASS_PULL_CAPTION_REFRESH = 'yyan-pull-caption-refresh';
    var CLASS_PULL_CAPTION_NOMORE = 'yyan-pull-caption-nomore';

    var CLASS_ICON = 'yyan-icon';
    var CLASS_SPINNER = 'yyan-spinner';
    var CLASS_ICON_PULLDOWN = 'yyan-icon-pulldown';

    var CLASS_BLOCK = 'yyan-block';
    var CLASS_HIDDEN = 'yyan-hidden';
    var CLASS_VISIBILITY = 'yyan-visibility';
...
(function($, window, document, undefined) {
	var CLASS_SCROLL = 'yyan-scroll';
	var CLASS_SCROLLBAR = 'yyan-scrollbar';
	var CLASS_INDICATOR = 'yyan-scrollbar-indicator';
	var CLASS_SCROLLBAR_VERTICAL = CLASS_SCROLLBAR + '-vertical';
	var CLASS_SCROLLBAR_HORIZONTAL = CLASS_SCROLLBAR + '-horizontal';

	var CLASS_ACTIVE = 'yyan-active';
	...
	var Scroll = $.Class.extend({
		...
		_initEvent: function(detach) {
			...
			var segmentedControl = this.wrapper.querySelector('.yyan-segmented-control');
			if (segmentedControl) { //靠，这个bug排查了一下午，阻止hash跳转，一旦hash跳转会导致可拖拽选项卡的tab不见
				yyan(segmentedControl)[detach ? 'off' : 'on']('click', 'a', $.preventDefault);
			}
	...
	$.fn.scroll = function(options) {
		...
		this.each(function() {
			...
			if (!id) {
				...
				if (self.classList.contains('yyan-segmented-control')) {
					_options = $.extend(_options, {
						scrollY: false,
						scrollX: true,
						indicators: false,
						snap: '.yyan-control-item'
					});
				}
...
(function($, window, document, undefined) {
    ...
    var PullRefresh = $.Scroll.extend($.extend({
        ...
        disablePullupToRefresh: function() {
            this._initPullupRefresh();
            this.bottomPocket.className = 'yyan-pull-bottom-pocket' + ' ' + CLASS_HIDDEN;
            this.wrapper.removeEventListener('scrollbottom', this);
        },
        disablePulldownToRefresh: function() {
            this._initPulldownRefresh();
            this.topPocket.className = 'yyan-pull-top-pocket' + ' ' + CLASS_HIDDEN;
 ...
 */
(function($, window) {
	var CLASS_SLIDER = 'yyan-slider';
	var CLASS_SLIDER_GROUP = 'yyan-slider-group';
	var CLASS_SLIDER_LOOP = 'yyan-slider-loop';
	var CLASS_SLIDER_INDICATOR = 'yyan-slider-indicator';
	var CLASS_ACTION_PREVIOUS = 'yyan-action-previous';
	var CLASS_ACTION_NEXT = 'yyan-action-next';
	var CLASS_SLIDER_ITEM = 'yyan-slider-item';

	var CLASS_ACTIVE = 'yyan-active';

	var SELECTOR_SLIDER_ITEM = '.' + CLASS_SLIDER_ITEM;
	var SELECTOR_SLIDER_INDICATOR = '.' + CLASS_SLIDER_INDICATOR;
	var SELECTOR_SLIDER_PROGRESS_BAR = '.yyan-slider-progress-bar';
	...
	var Slider = $.Slider = $.Scroll.extend({
		...
		_handleSlide: function(e) {
			...
			if (!self.wrapper.classList.contains('yyan-segmented-control')) {
				for (var i = 0, len = items.length; i < len; i++) {
					var item = items[i];
					if (item.parentNode === self.scroller) {
						if (i === _slideNumber) {
			...
			var indicatorWrap = self.wrapper.querySelector('.yyan-slider-indicator');
			if (indicatorWrap) {
				if (indicatorWrap.getAttribute('data-scroll')) { //scroll
					$(indicatorWrap).scroll().gotoPage(detail.slideNumber);
				}
				var indicators = indicatorWrap.querySelectorAll('.yyan-indicator');
				if (indicators.length > 0) { //图片轮播
					for (var i = 0, len = indicators.length; i < len; i++) {
						indicators[i].classList[i === detail.slideNumber ? 'add' : 'remove'](CLASS_ACTIVE);
					}
				} else {
					var number = indicatorWrap.querySelector('.yyan-number span');
					if (number) { //图文表格
						number.innerText = (detail.slideNumber + 1);
					} else { //segmented controls
						var controlItems = indicatorWrap.querySelectorAll('.yyan-control-item');
	...
	$.ready(function() {
		//		setTimeout(function() {
		$('.yyan-slider').slider();
		$('.yyan-scroll-wrapper.yyan-slider-indicator.yyan-segmented-control').scroll({
			scrollY: false,
			scrollX: true,
			indicators: false,
			snap: '.yyan-control-item'
...
(function($, document) {
    ...
    $.plusReady(function() {
        ...
        var CLASS_PLUS_PULLREFRESH = 'yyan-plus-pullrefresh';
        var CLASS_VISIBILITY = 'yyan-visibility';
        var CLASS_HIDDEN = 'yyan-hidden';
        var CLASS_BLOCK = 'yyan-block';
        ...
        var PlusPullRefresh = $.Class.extend({
                        ...
                        webview.addEventListener("dragBounce", function(e) {
                            ...
                            switch (e.status) {
                                ...
                                case "dragEndAfterChangeOffset": //正在刷新状态
                                    ...
                                    webview.evalJS("window.yyan&&yyan.options.pullRefresh.down.callback()");
            ...
            endPulldown: function() {
                ...
                if (_wv.parent() && this.options.down.style !== "circle") {
                    _wv.parent().evalJS("yyan&&yyan(document.querySelector('.yyan-content')).pullRefresh('" + JSON.stringify({
                        webviewId: _wv.id
...
(function($, window, document, name) {
	var CLASS_OFF_CANVAS_LEFT = 'yyan-off-canvas-left';
	var CLASS_OFF_CANVAS_RIGHT = 'yyan-off-canvas-right';
	var CLASS_ACTION_BACKDROP = 'yyan-off-canvas-backdrop';
	var CLASS_OFF_CANVAS_WRAP = 'yyan-off-canvas-wrap';

	var CLASS_SLIDE_IN = 'yyan-slide-in';
	var CLASS_ACTIVE = 'yyan-active';


	var CLASS_TRANSITIONING = 'yyan-transitioning';

	var SELECTOR_INNER_WRAP = '.yyan-inner-wrap';
	...
	var OffCanvas = $.Class.extend({
		init: function(element, options) {
			...
			if (this.scroller) {
				...
				document.body.classList.add('yyan-fullscreen'); //fullscreen
				this.refresh();
				this.initEvent();
			}
		},
		...
		refresh: function(offCanvas) {
			...
			this.scalable = this.classList.contains('yyan-scalable') && !this.slideIn;
			this.scroller = this.wrapper.querySelector(SELECTOR_INNER_WRAP);
		...
		initEvent: function() {
			...
			if (this.classList.contains('yyan-draggable')) {
	...
	$.ready(function() {
		$('.yyan-off-canvas-wrap').offCanvas();
	});
})(yyan, window, document, 'offcanvas');
/**
 * actions
 * @param {type} $
 * @param {type} name
...
(function($, name) {
	...
	var handle = function(event, target) {
		...
		if (className && ~className.indexOf(CLASS_ACTION)) {
			if (target.classList.contains('yyan-action-back')) {
				event.preventDefault();
			}
			return target;
		}
...
(function($, window, document, name) {
	...
	window.addEventListener('tap', function(event) {
		if ($.targets.modal) {
			...
			$.targets.modal.classList.toggle('yyan-active');
		}
	});
})(yyan, window, document, 'modal');
/**
 * Popovers
 * @param {type} $
 * @param {type} window
...
(function($, window, document, name) {
	...
	var CLASS_POPOVER = 'yyan-popover';
	var CLASS_POPOVER_ARROW = 'yyan-popover-arrow';
	var CLASS_ACTION_POPOVER = 'yyan-popover-action';
	var CLASS_BACKDROP = 'yyan-backdrop';
	var CLASS_BAR_POPOVER = 'yyan-bar-popover';
	var CLASS_BAR_BACKDROP = 'yyan-bar-backdrop';
	var CLASS_ACTION_BACKDROP = 'yyan-backdrop-action';
	var CLASS_ACTIVE = 'yyan-active';
	var CLASS_BOTTOM = 'yyan-bottom';
	...
	var togglePopover = function(popover, anchor, state) {
		...
		var _popover = document.querySelector('.yyan-popover.yyan-active');
		if (_popover) {
...
(function($, window, document, name, undefined) {
    ...
    var CLASS_CONTROL_ITEM = 'yyan-control-item';
    var CLASS_SEGMENTED_CONTROL = 'yyan-segmented-control';
    var CLASS_SEGMENTED_CONTROL_VERTICAL = 'yyan-segmented-control-vertical';
    var CLASS_CONTROL_CONTENT = 'yyan-control-content';
    var CLASS_TAB_BAR = 'yyan-bar-tab';
    var CLASS_TAB_ITEM = 'yyan-tab-item';
    var CLASS_SLIDER_ITEM = 'yyan-slider-item';
...
(function($, window, name) {
	...
	var CLASS_SWITCH = 'yyan-switch';
	var CLASS_SWITCH_HANDLE = 'yyan-switch-handle';
	var CLASS_ACTIVE = 'yyan-active';
	var CLASS_DRAGGING = 'yyan-dragging';

	var CLASS_DISABLED = 'yyan-disabled';
...
(function($, window, document) {
	...
	var CLASS_ACTIVE = 'yyan-active';
	var CLASS_SELECTED = 'yyan-selected';
	var CLASS_GRID_VIEW = 'yyan-grid-view';
	var CLASS_RADIO_VIEW = 'yyan-table-view-radio';
	var CLASS_TABLE_VIEW_CELL = 'yyan-table-view-cell';
	var CLASS_COLLAPSE_CONTENT = 'yyan-collapse-content';
	var CLASS_DISABLED = 'yyan-disabled';
	var CLASS_TOGGLE = 'yyan-switch';
	var CLASS_BTN = 'yyan-btn';

	var CLASS_SLIDER_HANDLE = 'yyan-slider-handle';
	var CLASS_SLIDER_LEFT = 'yyan-slider-left';
	var CLASS_SLIDER_RIGHT = 'yyan-slider-right';
	var CLASS_TRANSITIONING = 'yyan-transitioning';
	...
	var radioOrCheckboxClick = function(event) {
		...
		if (classList.contains('yyan-radio')) {
		...
		} else if (classList.contains('yyan-checkbox')) {
	...
	//fixed hashchange(android)
	window.addEventListener($.EVENT_CLICK, function(e) {
		if (cell && cell.classList.contains('yyan-collapse')) {
			e.preventDefault();
		}
	});
	...
	window.addEventListener('tap', function(event) {
		...
		if (classList.contains('yyan-collapse') && !cell.parentNode.classList.contains('yyan-unfold')) {
			if (!preventDefaultException.test(event.target.tagName)) {
				event.detail.gesture.preventDefault();
			}

			if (!classList.contains(CLASS_ACTIVE)) { //展开时,需要收缩其他同类
				var collapse = cell.parentNode.querySelector('.yyan-collapse.yyan-active');
				if (collapse) {
...
(function($, window) {
	...
	$.toast = function(message,options) {
		...
		if ($.os.plus && options.type !== 'div') {
			...
			toast.classList.add('yyan-toast-container');
			toast.innerHTML = '<div class="' + 'yyan-toast-message' + '">' + message + '</div>';
 ...
 */
(function($, window, document) {
    var CLASS_POPUP = 'yyan-popup';
    var CLASS_POPUP_BACKDROP = 'yyan-popup-backdrop';
    var CLASS_POPUP_IN = 'yyan-popup-in';
    var CLASS_POPUP_OUT = 'yyan-popup-out';
    var CLASS_POPUP_INNER = 'yyan-popup-inner';
    var CLASS_POPUP_TITLE = 'yyan-popup-title';
    var CLASS_POPUP_TEXT = 'yyan-popup-text';
    var CLASS_POPUP_INPUT = 'yyan-popup-input';
    var CLASS_POPUP_BUTTONS = 'yyan-popup-buttons';
    var CLASS_POPUP_BUTTON = 'yyan-popup-button';
    var CLASS_POPUP_BUTTON_BOLD = 'yyan-popup-button-bold';
    var CLASS_POPUP_BACKDROP = 'yyan-popup-backdrop';
    var CLASS_ACTIVE = 'yyan-active';
...
(function($, document) {
	var CLASS_PROGRESSBAR = 'yyan-progressbar';
	var CLASS_PROGRESSBAR_IN = 'yyan-progressbar-in';
	var CLASS_PROGRESSBAR_OUT = 'yyan-progressbar-out';
	var CLASS_PROGRESSBAR_INFINITE = 'yyan-progressbar-infinite';

	var SELECTOR_PROGRESSBAR = '.yyan-progressbar';
	...
	/**
	 * 创建并显示进度条
	 * @param {Object} container  可选，默认body，支持selector,DOM Node,yyan wrapper
	...
	$.fn.progressbar = function(options) {
		...
		this.each(function() {
			var self = this;
			var progressbarApi = self.yyan_plugin_progressbar;
			if (!progressbarApi) {
				self.yyan_plugin_progressbar = progressbarApi = {
					options: options,
...
(function($, window, document) {
	var CLASS_ICON = 'yyan-icon';
	var CLASS_ICON_CLEAR = 'yyan-icon-clear';
	var CLASS_ICON_SPEECH = 'yyan-icon-speech';
	var CLASS_ICON_SEARCH = 'yyan-icon-search';
	var CLASS_ICON_PASSWORD = 'yyan-icon-eye';
	var CLASS_INPUT_ROW = 'yyan-input-row';
	var CLASS_PLACEHOLDER = 'yyan-placeholder';
	var CLASS_TOOLTIP = 'yyan-tooltip';
	var CLASS_HIDDEN = 'yyan-hidden';
	var CLASS_FOCUSIN = 'yyan-focusin';
		...
		if (this.sliderActionClass) {
			...
			if (this.searchActionClass) {
				element.addEventListener('focus', function() {
					element.parentNode.classList.add('yyan-active');
				});
				element.addEventListener('blur', function() {
					if (!element.value.trim()) {
						element.parentNode.classList.remove('yyan-active');
					}
				});
			}
		}
	...
	Input.prototype.passwordActionClick = function(event) {
		...
		this.passwordAction.classList.toggle('yyan-active');
		event.preventDefault();
	};
	...
	$.fn.input = function(options) {
		...
		this.each(function() {
			...
			if (this.type === 'range' && row.classList.contains('yyan-input-range')) {
				actions.push('slider');
			} else {
				var classList = this.classList;
				if (classList.contains('yyan-input-clear')) {
					actions.push('clear');
				}
				if (!($.os.android && $.os.stream) && classList.contains('yyan-input-speech')) {
					actions.push('speech');
				}
				if (classList.contains('yyan-input-password')) {
					actions.push('password');
				}
				if (this.type === 'search' && row.classList.contains('yyan-search')) {
					actions.push('search');
				}
			}
...
(function($, window) {
    ...
    var Transparent = function(element, options) {
        ...
        } else if (!~this.scrollByElem.className.indexOf('yyan-scroll-wrapper')) {
            this.isNativeScroll = true;
        }

        this._style = this.element.style;
        this._bgColor = this._style.backgroundColor;
        var color = getColor(yyan.getStyles(this.element, 'backgroundColor'));
        if (color.length) {
            this._R = color[0];
            this._G = color[1];
            this._B = color[2];
    ...
    Transparent.prototype.destory = function() {
        ...
        this.element.yyan_plugin_transparent = null;
    };
    $.fn.transparent = function(options) {
        options = options || {};
        var transparentApis = [];
        this.each(function() {
            var transparentApi = this.yyan_plugin_transparent;
            if (!transparentApi) {
                ...
                transparentApi = this.yyan_plugin_transparent = new Transparent(this, options);
            }
    ...
    $.ready(function() {
        $('.yyan-bar-transparent').transparent();
    });
})(yyan, window);
/**
 * 数字输入框
...
(function($) {
    ...
    var holderClassName = 'yyan-numbox';
    var plusClassSelector = '.yyan-btn-numbox-plus,.yyan-numbox-btn-plus';
    var minusClassSelector = '.yyan-btn-numbox-minus,.yyan-numbox-btn-minus';
    var inputClassSelector = '.yyan-input-numbox,.yyan-numbox-input';
...
(function($, window, document) {
    ...
    var defaultOptions = {
        ...
        loadingIcon: 'yyan-spinner' + ' ' + 'yyan-spinner-white', //图标，可为空
    ...
    var Button = function(element, options) {
        ...
        if (this.options.loadingIcon === null) {
            this.options.loadingIcon = 'yyan-spinner';
            if ($.getStyles(this.element, 'color') === 'rgb(255, 255, 255)') {
                this.options.loadingIcon += ' ' + 'yyan-spinner-white';
            }
        }
    ...
    $.fn.button = function(state) {
        var buttonApis = [];
        this.each(function() {
            var buttonApi = this.yyan_plugin_button;
            if (!buttonApi) {
                ...
                this.yyan_plugin_button = buttonApi = new Button(this, {
